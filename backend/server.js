...
app.post("/crear_nota", async (req, res) => {
  ...
  if (req.body.titulo_nota != "" && req.body.cuerpo_nota != "") {
    await pool.query(
      `INSERT INTO notas (titulo, cuerpo, ID_creador) VALUES ('${req.body.titulo_nota}', '${cuerpo_nota_purificado}', 1);`
    );
    res.send({
      status: "exito",
      titulo: "Exito",
...
app.post("/editar_nota", async (req, res) => {
  ...
  try {
    await pool.query(
      `UPDATE notas SET cuerpo = '${req.body.cuerpo_editado}' WHERE ID_nota = ${id_nota};`
    );
    res.send({
      status: "exito",
      titulo: "Exito",
...
app.get("/obtener_notas", async (req, res) => {
  const [notas] = await pool.query(
    "SELECT ID_nota, titulo, cuerpo, fecha_creacion FROM notas"
  );
  res.send(notas);
});
